...
// We will replace the entire block from "// Simple encode/decode" up to "// ── Determine which screen to show ──"
...
`;
// Put it right before "// ===== Master Data Arrays ====="
...
content = content.replace("rcvVerifyBtn.addEventListener('click', function() {", "rcvVerifyBtn.addEventListener('click', async function() {"); // catch variants

// 8. Inject array saves inside render UI updates so we guarantee persistence on any mutate
...
// Make sure not to infinite loop or save excessively on init
// Wait, calling save inside the render function will save 1 time on boot for every table. That's harmless.
